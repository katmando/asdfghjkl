...
int main(void) {
	...
	_delay_ms(125);						
	float adc_tempf;					
	USART_putstring("AT+CWJAP=\"WIFI_NAME_HERE\",\"WIFI_PASSWORD_HERE\"");	
	while (1) {												
		if (TCNT1 == OCR1A) {								
			read_adc();										
			adc_tempf = (ADCH << 8) + ADCL;					
			adc_tempf = (9/5)*adc_tempf + 32;				
...
void Timer1_init(void) {					// Function to Initialize Timer1 properties
	...
	TCCR1B = (1<<CS12)|(0<<CS11)|(1<<CS10);	// Set Clock Select '1024'
}
...
void adc_init(void) {
	...
	(1<<ADPS2)|(0<<ADPS1)|(1<<ADPS0);		// ADC  Select Bits '32'
}
